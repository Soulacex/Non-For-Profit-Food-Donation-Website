...
public class ManagerFileDAO implements ManagerDAO{
     ...
     */
    public ManagerFileDAO(@Value("${Cupboard.file}") String filename,ObjectMapper objectMapper) throws IOException {
    ...
    ** {@inheritDoc}
     */
    @Override
    public Needs addFoodNeed(String name, double cost, String type, Integer quantity) throws IOException{
        synchronized(Needs) {
    ...
    ** {@inheritDoc}
     */
    @Override
    public boolean editFoodNeed(String name, double newCost, String newType, Integer newQuantity) throws IOException {
        synchronized(Needs) {
            if (Needs.containsKey(name) == false)
                return false;  // hero does not exist
            Needs newNeed = Needs.get(name);
                if (newCost >= 0.0) {
                    newNeed.setCost(newCost);
                } 

                if (newType != null) {
                    newNeed.setType(newType);    
                }
    ...
    ** {@inheritDoc}
     */
    @Override
    public boolean removeFoodNeed(String name) throws IOException {
        synchronized(Needs) {
